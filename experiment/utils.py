...
def setup_experiment():
    ...
    try:
        subject = Subject(name=subject,
                          ...
                          sex=sex,
                          cohort=cohort)
        subject.data_path = os.path.join(get_config("DATA_ROOT"), f"{subject}.h5")
        subject.add_subject_to_h5file(os.path.join(get_config("SUBJECT_ROOT"), f"{subject}.h5"))
        # test_subject.file_path
    except ValueError:
        # read the subject information
        sl = SubjectList(file_path=os.path.join(get_config("SUBJECT_ROOT"), f"{subject}.h5"))
        sl.read_from_h5file()
