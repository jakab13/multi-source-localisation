...
class RP2RX8SpeakerCalSetting(DeviceSetting):
    ...
    device_freq = CFloat(48828.125, group='status', dsec='sampling frequency of the device (Hz)')
    rcx_file_RP2 = Str('RCX\\RP2_speaker_calibration.rcx', group='status', dsec='the rcx file for RP2')
...
class RP2RX8SpeakerCal(Device):
    ...
    def _initialize(self, **kwargs):
        ...
        self.RX81 = tdt.initialize_processor(processor=self.setting.processor_RX8,
                                             connection=self.setting.connection,
                                             index=1,
                                             path=os.path.join(expdir, self.setting.rcx_file_RX8))
        self.RX82 = tdt.initialize_processor(processor=self.setting.processor_RX8,
                                             connection=self.setting.connection,
                                             index=2,
                                             path=os.path.join(expdir, self.setting.rcx_file_RX8))
    ...
    def read(self, start_idx=0, nsamples=None, dtype=np.float32):
        ...
        """
        if not nsamples:
            nsamples = self.RP2.GetTagVal('recBuf_idx')
        # the ReadTagV treats each 32-bit buffer as floats
        data = np.array(self.RP2.ReadTagV('audio_data', start_idx, nsamples), dtype=np.float32)
    ...
    def load_filters(self, coefs, nTaps):
        ...
        tdt.set_variable('filter_coefs', coefs, self.RX81)
        tdt.set_variable('filter_coefs', coefs, self.RX82)
